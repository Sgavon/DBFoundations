...
-- 2021-11-21,SalvadorGavonel,Created File
-- 2021-11-21,SalvadorGavonel,Created DB
-- 2021-11-21,SalvadorGavonel,Answered Questions
-- 2021-11-21,SalvadorGavonel,switched tables for base views for Q3-Q10
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_SalvadorGavonel')
	 Begin 
	  Alter Database [Assignment06DB_SalvadorGavonel] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_SalvadorGavonel;
	 End
	Create Database Assignment06DB_SalvadorGavonel;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_SalvadorGavonel;
...
Select
	...
	,InventoryDate as 'Inventory'
	,Count as 'Inventory Count'
from vProducts P
	Inner Join vInventories I on P.ProductID = I.ProductID
Go
Select * from vInventoriesByProductsByDates Order By Inventory, Product, [Inventory Count]
go
...
AS
Select
	Distinct InventoryDate as 'Inventory Date'
	,EmployeeFirstName + ' ' + EmployeeLastName as 'Employee'
from vInventories I
	Inner Join vEmployees E on I.EmployeeID = E.EmployeeID
go

Select * from vInventoriesByEmployeesByDates Order By [Inventory Date]
go
...
from vCategories C
	...
	Inner Join vInventories I on P.ProductID = I.ProductID
go
Select * from vInventoriesByProductsByCategories Order by Category, Product, Inventory, [Inventory Count]
go
...
from vCategories C
	...
	Inner Join vEmployees E on I.EmployeeID = E.EmployeeID
go
Select * from vInventoriesByProductsByEmployees Order by [Inventory Date], Category, Product, [Employee]
go
...
from vCategories C
	...
	Inner Join vInventories I on P.ProductID = I.ProductID
	Inner Join vEmployees E on I.EmployeeID = E.EmployeeID
where P.ProductID in (Select Q.ProductID from vProducts Q where ProductName in ('Chai', 'Chang'))
go

Select * from vInventoriesForChaiAndChangByEmployees Order by [Inventory Date], Category, Product, [Employee]
go
...
from vEmployees as E
	Inner Join vEmployees as Mgr on E.ManagerID = Mgr.EmployeeID
go
...
Select C.CategoryID,
	...
	from vCategories C
		...
		join vEmployees M on M.EmployeeID = E.ManagerID
go
